...
function AddCourse($connection, $CourseType, $Instructors1, $Instructors2, $Instructors3, $Price, $NumberofSession, $Student, $StudentPassword) {
   ...
   $query1 = "INSERT INTO `Course` (`M_ID`, `Price`, `Course_Type`, `Number_of_Session`, `Remaining_Number` ) VALUES ('$AC_S', '$AC_P', '$AC_CT', '$AC_NS', '$AC_NS');";
   
   if(!mysqli_query($connection, $query1)) echo("<p>Error adding Course data.</p>");
   
   $query2 = "INSERT INTO `Compose` (`I_ID`, `Course_ID`) VALUES ('$AC_I1', LAST_INSERT_ID()), ('$AC_I2', LAST_INSERT_ID()), ('$AC_I3', LAST_INSERT_ID());";
   
   if(!mysqli_query($connection, $query2)) echo("<p>Error adding Compose data.</p>");
}
...
$StudentPassword = htmlentities($_POST['StudentPassword']);
	...
	if (strlen($Instructors1) || strlen($Price)) {
